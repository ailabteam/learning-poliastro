# project_ga_final_for_paper_english.py (ENHANCED PLOT STYLING)
...
# ===================================================================
SCENARIO = 'FAR_ORBIT' # <-- CHANGE TO 'GEO' TO RUN THE OTHER SCENARIO
...
toolbox.register("attr_rb_ratio", random.uniform, 1.01, 500.0)
...
def main():
    ...
    # Use HTML <b> tags for bold text in labels
    ...
    fig_wide.add_trace(go.Scatter3d(x=hoh_coords.x.to_value(u.km), y=hoh_coords.y.to_value(u.km), z=hoh_coords.z.to_value(u.km), mode='lines', line=dict(color='cyan', width=6, dash='dash'), name=f'<b>Hohmann (ΔV: {hohmann_cost.to(u.km/u.s):.2f} km/s)</b>'))
    ...
    fig_wide.update_layout(
        title=dict(text=f"<b>Optimal Trajectory Comparison: {SCENARIO} (Wide View)</b>", x=0.5, font=dict(size=24)),
        legend=dict(title=dict(text="<b>Trajectories</b>", font=dict(size=18)), font=dict(size=16)),
        scene=dict(
            xaxis=dict(title=dict(text='<b>X (km)</b>', font=dict(size=16))),
            yaxis=dict(title=dict(text='<b>Y (km)</b>', font=dict(size=16))),
            zaxis=dict(title=dict(text='<b>Z (km)</b>', font=dict(size=16))),
            aspectmode='cube'
        ),
    ...
    fig_wide.write_image(output_filename_wide, width=2000, height=1500, scale=2) # Higher resolution
    print(f"-> Wide view plot saved to: {output_filename_wide}")

    # --- ZOOM VIEW PLOT (ONLY FOR FAR_ORBIT) ---
    if SCENARIO == 'FAR_ORBIT':
        ...
        plotter_zoom.plot(hoh_transfer, label=f'<b>Hohmann (ΔV: {hohmann_cost.to(u.km/u.s):.2f} km/s)</b>', color="cyan")
        ...
        fig_zoom.update_layout(
            title=dict(text=f"<b>Optimal Trajectory Comparison: {SCENARIO} (Zoomed-in View)</b>", x=0.5, font=dict(size=24)),
            legend=dict(title=dict(text="<b>Trajectories</b>", font=dict(size=18)), font=dict(size=16)),
            scene=dict(
                xaxis=dict(title=dict(text='<b>X (km)</b>', font=dict(size=16))),
                yaxis=dict(title=dict(text='<b>Y (km)</b>', font=dict(size=16))),
        ...
        fig_zoom.write_image(output_filename_zoom, width=2000, height=1500, scale=2)
